...
#Natural constant
GG =  6.67430e-8  # gravitational constant  [dyn cm^2 g^-2]
...
shadowed_length = R[len(np.where (R >=H_cg*R_rim/H_rim)[0])] -R[0]
...
#-0.3 0.38343241097708336 0.046270833860451235 0.616693713123714
#0    0.29863670625168576 0.0321643566662279   0.2916241471066627
#0.3   0.17377332077474755 0.014617283481862628 0.019691837032515546


"""

0.06850714268787533/0.5036194069000989     , 0.0639605571613923/0.48037274073022934
Out[2]: (0.13602959248443902, 0.13314776576240336)

0.06068158356157809/0.46328861789440673, 0.05412650229824152/0.4282479867738147
Out[3]: (0.13098008718057627, 0.12639055867139243)"""
...
ax.set_xlabel("R (AU)")
ax.set_ylabel("$T_{mid}$ (K)")
plt.axvline(x=R_rim/AU, color='b', linestyle=':')
# Add a label next to the line
plt.text(R_rim/AU + 0.01, 150, 'R_rim', rotation=90)

plt.axvline(x=shadowed_length/AU +R_rim/AU, color='b', linestyle=':')
# Add a label next to the line
plt.text(shadowed_length/AU +R_rim/AU+ 0.01, 150, 'R_flare', rotation=90)
plt.show()   
...
plt.axvline(x=R_rim/AU, color='b', linestyle=':')
# Add a label next to the line
plt.text(R_rim/AU + 0.01, 0.4, 'R_rim', rotation=90)

plt.axvline(x=shadowed_length/AU +R_rim/AU, color='b', linestyle=':')
# Add a label next to the line
plt.text(shadowed_length/AU +R_rim/AU+ 0.01, 0.4, 'R_flare', rotation=90)
#plt.text(H_cg[nr-1]/R[nr-1], 0.4, 'Hs/R', rotation=90)
