...
# grid parameters 
nr = 10  # Number of radial grids. 
ntheta = 257  # Number of grids in elevation.
nz = 10 # Number of elevation grids. 
...
def iterative_newton2(fun, x_init, jacobian):
    max_iter = 5000
    epsilon = 1e-8
...
for i, (r_i, s_i) in enumerate(zip(R, sigma)):
    ...
    #X_cg, H_cg, hcg, Ti, alpha
    print(x_init_disk)
    x_sol_disk = iterative_newton2(disk_function,x_init_disk,jacobian_disk)
    print(x_sol_disk)
    ...
    #print(x_init_disk)
    #print(x_sol_disk)
    #print(X_cg[i], H_cg[i], h_cg[i],Ti[i], impinge_angle[i])

"""


x_init_disk = [X_interpolate[0] , H_interpolate[0], h_interpolate[0],Ti_interpolate[0],alpha_interpolate[0]]
R = R[0]
sigma = sigma[0]
x_sol_disk = iterative_newton(disk_function, x_init_disk, jacobian_disk)
print("I am solution for disk", x_sol_disk)
#print('solution exercice:', x_sol )