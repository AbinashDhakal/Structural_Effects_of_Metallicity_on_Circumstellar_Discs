...
def Rim_radial(H_rim, Rrim_guess):
    return (lstar/(4*np.pi*T_rim**4 *ss))**0.5 *(1+ H_rim/Rrim_guess)**0.5
...
def converging_rim():
    ...
    H_rim =0.1162*AU
    iteration =0
    while np.round(R_guess/AU, 7) != np.round(R_rim/AU,7):
        R_guess =        R_rim
        w = 0.9999999
        error = 1*AU
        while error >10e-9 *AU:
            temp_r = R_rim
            R_rim = (w*R_rim +(1-w)*Rim_radial(H_rim, R_rim))
...
def converging_rim():
    ...
    H_rim =0.1162*AU
    iteration =0
    while np.round(H_guess/AU, 7) != np.round(H_rim/AU,7):
        H_guess =        H_rim
        w = 0.9999999
        error = 1*AU
        while error >0.00000001*AU:
            temp_r = R_rim
            R_rim = (w*R_rim +(1-w)*Rim_radial(H_rim, R_rim))
            error = np.max(np.abs(temp_r-R_rim))
            print(R_rim/AU, temp_r/AU)
...
"""
R_rim, H_rim, h_rim, sigma_rim, X_rim = converging_rim()
print(R_rim/AU,H_rim/AU )   
