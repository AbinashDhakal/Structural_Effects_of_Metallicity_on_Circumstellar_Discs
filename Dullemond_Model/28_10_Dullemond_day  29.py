...
# Define your function. First variable is your input var and all vars after that are the variables to be trained
def sigmoid(x, a, b):
     y = 1 / (1 + np.exp(-a*(x-b)))
     return y

# Uses function "sigmoid" that is defined above as the function to be used for your best fit curve.
# Returns trained a, b, c values as an array. Stored as "popt"
popt, pcov = curve_fit(sigmoid,sample_data.R, sample_data.Ti)
a, b = popt
...
fig, ax = plt.subplots()
ax.plot(R/AU, Ti_interpolate)
#ax.plot(R/AU, sigmoid(R/AU,*popt))
...
for z in range(Simulation):
    print(" I am running " ,z, " simulation")
    for i, (r_i, H_i, h_i, T_i,X_i, a_i, s_i) in enumerate(zip(R, H_interpolate, h_interpolate, Ti_interpolate, X_interpolate,alpha_interpolate, sigma)):
        ...
        const_temp = (impinge_angle[i]*psi_s/2/psi_i)**0.25 *(rstar/r_i)**0.5 *tstar
        #print("Const temperaureate is ",const_temp)
        Ti[i]= fsolve(temp_interior,T_i*0.95, args=const_temp)
        
        
        const_hcg = (Ti[i]/t_virial)**0.5 * (r_i/rstar)**0.5 *r_i
        ...
        const_X = 2*impinge_angle[i]/kp_stellar/s_i
        X_cg[i] = fsolve(chi_disk, X_i*0.95, args = const_X)
        
        const_Hcg = X_cg[i] * h_cg[i]
...
for z in range(Simulation):
    ...
    for i, (r_i, H_i, h_i, T_i,X_i, a_i, s_i) in enumerate(zip(R, H_interpolate, h_interpolate, Ti_interpolate, X_interpolate,alpha_interpolate, sigma)):
       ...
       #print("Help3\n\n")
        ...
        #print(4)
       #print("Help4\n\n\n\n")
        X_i = const_X = 2*a_i/kp_stellar/s_i
        X_cg[i] = fsolve(chi_disk, X_i*0.95, args = const_X)
       #print("Hiiiiiiiiiiiiiiiiiiiiiiiiii")
