...
ax.set_ylabel("without_Interior_Temperature")
...
Pr = 1 #Prandtl number
visc_const =10**-2
hp = (T_effective/t_virial)**0.5 *(R/rstar)**0.5 *R #pressure scale
sigma = surface_density(R) #surface density 
cs = np.sqrt(kb*adia_gamma*T_effective/(mu*mp)) #sound speed
omega = np.sqrt(GG*mstar/R**3) #Keplerian velocity
cV = kb/(mu*mp*(adia_gamma-1)) #specific const volume 
cP = adia_gamma /cV #specific const pressure
cP = 5*gasR/2
rho = Rho(0,hp,sigma) #Density 
vt = visc_const*cs**2/omega #viscosity
kt = rho*vt*adia_gamma*cV/Pr #Turblent thermal conductivity
        #Discretising laplace equation
Q_visc = rho*vt*(9*GG*mstar/4/R**3)    
fig, ax = plt.subplots()
ax.loglog(R/AU, kt, marker = '*', markevery = (Q_stability <1))
ax.set_xlabel("Radius")
ax.set_ylabel("kt")
ax.set_title("kt vs. Radius")
plt.show()   


fig, ax = plt.subplots()
ax.loglog(R/AU, Q_visc, marker = '*', markevery = (Q_stability <1))
ax.set_xlabel("Radius")
