...
for (r_i,s_i) in zip(R,sigma):
    H_ccg = fsolve(H_cg_refine, H_rim,args = (r_i, s_i))
    print(H_ccg)
...
def Zo(z,h_rim, sigma):
    def intergrand(x):
        return np.exp(-x**2)
    analytical_sol,error= integrate(intergrand,z/h_rim, np.inf)
    answer = (8*sigma*kp_stellar/(np.pi)**0.5)*analytical_sol
    return answer -1
Z0 = np.zeros(nr)
for i, s_i in enumerate(sigma):
    Z0[i] = fsolve(Zo,0.5, args = (h_rim, s_i))
