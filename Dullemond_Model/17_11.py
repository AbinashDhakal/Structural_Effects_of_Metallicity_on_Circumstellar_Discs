...
def irr_surface_Temp(R, T_guess):
    epsilon =80*(T_guess/tstar)**1.5
    T_s = (1/epsilon)**0.25 * (rstar/2/R)**0.5 *tstar
    w = 0.9999999
    error = 10
    while error >0.01:
        Temp = T_s
        epsilon =80*(Temp/tstar)**1.5
...
ax.set_title("Surface_tempeature vs. Radius")
plt.show()
...
Ti[0] = T_s[0] = 0
T_effective = T_s + Ti + T_visc


h_cg = pressure_height(T_effective,R)
X_cg= chi_disk(impinge_angle, sigma)
H_cg = X_cg*h_cg
