...
def func(H_ccg, R):
    def integrand(z,R):
        return 400*(R/AU)**-1.5 * np.exp(-z**2 / (2*H_ccg**2))
    integral, err = integrate(integrand, H_ccg,10*AU, args =(R,H_ccg))
    return (1 - integral* (0.4*rstar/R +(gamma - 1)*(H_ccg/R)))

for i, r_i in enumerate(R):
    H_cfg[i]=fsolve(func,H_rim,args = r_i)
"""    
#Method 2
def integrand(H_ccg,z,R):
    return 400*(R/AU)**-1.5 * np.exp(-z**2 / (2*H_ccg**2))

def func(H_ccg,z,R):
    integral, err = integrate(integrand, H_ccg,np.inf,args =(R,H_ccg))
    return 1 - integral* (0.4*rstar/R +(gamma - 1)*(H_ccg/R))

H_ccg = []
for i, r_i in enumerate(R):
    H_ccg[i] =fsolve(func,H_rim, args =r_i)
print("Help")
