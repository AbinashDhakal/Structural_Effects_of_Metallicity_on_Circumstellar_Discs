...
def innerHole(Ri,Metallicity, T_Stellar):
    ...
    while np.round(Tguess, 2) != np.round(Tgas,2):
        ...
        yrs = 3.154e+7 #second in years
        #M_acc[10^-5  - 10^-11 Myr^-1]
        #M_acc = 3.6*10**-9 *ms/yrs
        M_acc = 2*10**-7* ms/yrs  # https://www.aanda.org/articles/aa/pdf/2008/20/aa09641-08.pdf
        Cs = np.sqrt(kb*Tgas/mu/mp) #Sound speed
        kp = np.sqrt(GG*mstar/Ri**3) #Keplerian 
        Hp = Cs/kp
        vt = 10**-2 *Hp*Cs
        s_gas = M_acc/3/np.pi/vt *(1-(rstar/Ri)**0.5)
        Rho_gas = s_gas/np.sqrt(2*np.pi)/Hp
        P_gas = Rho_gas*kb*Tgas/mu/mp
...
print(kp_TgasTrad,Rho_gas, Tgas)
