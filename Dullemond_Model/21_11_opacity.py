...
Tau_intergrand =lambda R:sigma0*(R/AU)**(-1.5)   /(np.sqrt(2*np.pi *kb*R**3/(mu*mp*GG*mstar)))
Tau_itegral= integrate.quad(Tau_intergrand,np.inf , R_rim)
...
Tau = -0.25*np.sqrt(2/np.pi)*sigma/h_cg*R*7.7
#(modified_data[1][4])
def converging_disk(Tgas_guess, r_ii, Metallicity, T_stellar):
    psi_i = 1
    psi_s = 1 
    T_gas = 300
    #print(T_gas)
    data = np.loadtxt('planck_data1.csv', delimiter=',', skiprows=1)
    modified_data = np.loadtxt('planck_data1.csv', delimiter=',', skiprows=1,  usecols=(1, 2,3,4,5))
    while np.round(T_gas, 3) != np.round(Tgas_guess ,3):
    #while alpha_guess !=a_i:
        Tgas_guess = T_gas
        print(T_gas)
        h_gas = pressure_height(T_gas,r_ii)
        sigma_gas = surface_density(r_ii)
        Rho_gas = Rho(0, h_gas, sigma_gas)
        P_gas = GasPressure(Rho_gas, T_gas)
        #print("printing pressute",P_gas,Rho_gas)
        # Vector we try to match with data
        search_vect =  np.array([Metallicity, T_stellar, T_gas ,P_gas, Rho_gas])
        ...
        Tau = 0.5*kp_Tgas*Rho_gas
        Tau_eff = (0.433 +3*Tau/8 +1/4/Tau)
        print(Tau_eff)
        T_gas = T_stellar*(Tau_eff)**0.25
        ...
        #Tau_itegral = integrate.quad(Tau_intergrand,np.inf , r_ii)
        #print(Tau_itegral[0],kp_Tgas,T_gas)
        #Tau = -kp_Tgas*Tau_itegral[0]/np.sqrt(T_gas)
        #Tau = -0.25*np.sqrt(2/np.pi)*sigma_gas/h_gas*r_ii*kp_Tgas
        #print("printing tau value", Tau)

        #T_gas =((3/4)*T_stellar**4 * (Tau + 2/3))
        #print(T_gas)
    return  T_gas,Tau

print(converging_disk(Tgas_guess, rstar, Metallicity, T_stellar))
T_gas = np.zeros(nr)
Tau =  np.zeros(nr)
for i,(ri) in enumerate(R):
    T_gas[i],Tau[i] =converging_disk(Tgas_guess, ri, Metallicity, T_stellar)
    



fig, ax = plt.subplots()
ax.semilogx(R/AU, T_gas)
