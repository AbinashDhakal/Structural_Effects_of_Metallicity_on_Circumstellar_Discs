...
zi = np.linspace(0.0, H_cg.max(), nz +1)
...
plt.contour(Ra, Za,rho)
plt.colorbar()
plt.show()
"""

F_irr = impinge_angle*(lstar/(4*np.pi*R*R)) + 2*impinge_angle*ss*T_rim**4 *(R_rim/R)**2 / np.pi *np.cos(theta) * (delta * np.sqrt(1 - delta) + np.arcsin(delta))
...
Zed, Rad = np.meshgrid(z_elevation, R)
Zed, Sig = np.meshgrid(z_elevation, sigma)
Zed, H_mg = np.meshgrid(z_elevation, H_cg)

Rho = Sig * np.exp(-0.5 * Zed * Zed / (H_mg * H_mg)) / (H_mg * np.sqrt(2*np.pi))

fig, ax = plt.subplots()
pcol = ax.pcolormesh(Rad, Zed, Rho, shading="gouraud")
ax.set_title("Rho")
ax.set_xlabel("Radius")
ax.set_ylabel("Z")
fig.colorbar(pcol)
