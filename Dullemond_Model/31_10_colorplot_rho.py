...
X_grid = mp.meshgrid(X_cg)
impinge_angle_grid = np.meshgrid(impinge_angle)
H_grid = np.meshgrid(H_cg)
h_grid = np.meshgrid(h_cg)
sigma_grid = np.meshgrid(sigma)
Ti_grid = np.meshgrid(Ti)
...
for i, r_i in enumerate(R):
    ...
    Ti_grid[i] = np.meshgrid(Ti)
    h_grid[i] = np.meshgrid(h_cg)
    sigma_grid[i] = np.meshgrid(sigma)
    X_grid[i] = mp.meshgrid(X_cg)
    H_grid[i] = np.meshgrid(H_cg)

    impinge_angle_grid[i] = np.meshgrid(impinge_angle)
...
while error >0.0001:
    ...
    Ts = surface_Temp(epsilon, r_ii)
    psi_s= kp_interp(Ts)
    s_i = surface_density(r_ii)
    psi_i = s_i *kp_interp(Tii)
    Tii = disk_temp(0.19459029132621866, r_ii, psi_i, psi_s)
    error = np.abs(Ts -T_guess)
    print(T_guess, Ts,Tii)
    
def converging_temperature(Tsguess, r_ii, psi_s, psi_i, angle_i):
    T_s = 1200
    while np.round(T_s, -1) != np.round(Tsguess,-1):
    #while alpha_guess !=a_i:
        Tsguess = T_s
        epsilon =kp_interp(Tsguess)/ kp_interp(tstar)
        T_s = surface_Temp(epsilon, r_ii)
        s_i = surface_density(r_ii)
        psi_s = kp_interp(T_s)
        T_last = disk_temp(angle_i,r_ii, psi_s, psi_i )
        psi_i = kp_interp(T_last)
        T_i =disk_temp(angle_i, r_ii,psi_s, psi_i)
        #print(epsilon, T_s, psi_i, psi_s,T_i)
        #print(T_s, Tsguess)
    return T_s, T_i
